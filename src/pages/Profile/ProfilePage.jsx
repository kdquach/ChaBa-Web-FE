...
const ProfilePage = () => {
    ...
    const formattedAddress = useMemo(() => {
        try {
            if (Array.isArray(user?.addresses) && user.addresses.length > 0) {
                ...
                const parts = [a?.street, a?.ward?.name, a?.district?.name, a?.city?.name].filter(Boolean);
                return parts.join(", ");
            }
    ...
    const onSaveProfile = async (values) => {
        ...
        try {
            ...
            if (keyError && keyError[2].includes('Invalid')) {
                ...
                message.error(e?.response?.data?.message || "Cập nhật thông tin thất bại");
            }
        } finally {
    ...
    const onChangePassword = async (values) => {
        try {
            ...
            message.success("Đổi mật khẩu thành công! Bạn sẽ chuyển về trang login sau 3s");
            pwdForm.resetFields();
            setTimeout(() => {
                logout();
                navigate('/login')
    ...
    const beforeUpload = (file) => {
        const isAllowed = ["image/jpeg", "image/png", "image/jpg"].includes(file.type);
        if (!isAllowed) {
            ...
            <PageHeader
                ...
                extra={
                    <Space style={headerExtraGap}>
                        ...
                        <Button type="primary" onClick={() => form.submit()} loading={infoLoading}>
                            Lưu thay đổi
                        </Button>
                    </Space>
                }
                    ...
                    <Card style={sectionCardStyle} bodyStyle={{ padding: 20 }}>
                        ...
                        >
                            ...
                            <div style={{ marginTop: 8 }}>
                                <Tag color={user?.role === "admin" ? "red" : user?.role === "staff" ? "blue" : "green"} style={{ marginRight: 8 }}>
                                    {user?.role === "admin" ? "Quản trị viên" : user?.role === "staff" ? "Nhân viên" : "Khách hàng"}
                                </Tag>
                                <Tag color={user?.status === "active" ? "green" : "orange"}>{user?.status === "active" ? "Đang hoạt động" : "Ngừng hoạt động"}</Tag>
                            </div>
                        </div>
                        ...
                        <Descriptions
                            ...
                            colon
                            items={[
                                { key: "address", label: <span style={{ fontWeight: 500 }}>Địa chỉ</span>, children: formattedAddress || "—" },
                            ].filter(Boolean)}
                        />
                                ...
                                >
                                    ...
                                    <Form.Item style={{ textAlign: "right", marginBottom: -10 }}>
                                        <Space style={{ marginTop: 10, headerExtraGap }}>
                                            <Button type="primary" onClick={onSaveAddress} loading={addressLoading}>
                                                Lưu địa chỉ
                                            </Button>
                                        </Space>
                                    </Form.Item>
                ...
                <Col xs={24} lg={16}>
                    <Card style={sectionCardStyle} bodyStyle={{ padding: 20 }}>
                        ...
                        >
                            ...
                            ]} label={<span style={labelStyle}>Họ và tên</span>} name="name">
                                <Input placeholder="Tên người dùng" />
                            </Form.Item>
                            <Form.Item label={<span style={labelStyle}>Email</span>} name="email" rules={[{ type: "email", message: "Email không hợp lệ" }, { required: true, message: "Vui lòng nhập email" }]}>
                            ...
                            <Form.Item rules={[
                                ...
                                label={<span style={labelStyle}>Số điện thoại</span>} name="phone">
                    ...
                    {!user?.provider || user?.provider === "local" ? (
                        <Card style={sectionCardStyle} bodyStyle={{ padding: 20 }}>
                            <Title level={5} style={{ marginTop: 0 }}>Bảo mật</Title>
                            <Form
                                form={pwdForm}
                                layout={screens.lg ? "horizontal" : "vertical"}
                                ...
                                colon={false}
                            >
                                <Form.Item label={<span style={labelStyle}>Mật khẩu hiện tại</span>} name="currentPassword" rules={[{ required: true, message: "Vui lòng nhập mật khẩu hiện tại" }]}>
                                    <Input.Password placeholder="••••••••" />
                                </Form.Item>
                                <Form.Item label={<span style={labelStyle}>Mật khẩu mới</span>} name="newPassword"
                                        ...
                                        validator: (_, value) => {
                                            if (value && !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/.test(value)) {
                                                return Promise.reject(
                                                    new Error("Mật khẩu phải chứa ít nhất một chữ và một số")
                                                );
                                            }
                                ...
                                <Form.Item label={<span style={labelStyle}>Nhập lại mật khẩu</span>} name="confirmPassword" dependencies={["newPassword"]} rules={[
                                ...
                                <Form.Item style={{ marginBottom: 0 }}>
                                    <Space>
                                        <Button onClick={() => pwdForm.resetFields()}>Làm mới</Button>
                        ...
                        <Alert
                            ...
                            description={
                                <div>
                                    Tài khoản đăng nhập bằng mạng xã hội không có chức năng thay đổi mật khẩu.
                                </div>
                            }
                        />
                    </Card>}
